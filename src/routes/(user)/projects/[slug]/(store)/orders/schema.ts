...
import { z } from "zod";
const orderStatuses: StoreOrderStatus[] = ['aborted', 'approved', 'delivered', 'in_progress', 'pending_approval'];
...
export const formSchema = z.object({
  ...
  additional_discount: z.number({coerce: true, message: 'Must be between 0 to 100'}).optional().refine((discount) => discount && (discount >= 0 && discount <= 100)),
});
